...
class Attention(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        k = k.permute(0, 2, 1, 3)
        v = v.permute(0, 2, 1, 3)
        #print((q @ k.transpose(-2, -1) * self.scale).shape,self.attention_biases[:, self.attention_bias_idxs].shape,self.attention_biases.shape)
        attn = (
...
class Meta3D(nn.Module):
    ...
    def forward(self, x):
        if self.use_layer_scale:
           # print(x.shape,self.token_mixer(self.norm1(x)))
            x = x + self.drop_path(
...
def meta_blocks(dim, index, layers,
                ...
                use_layer_scale=False, layer_scale_init_value=1e-5, vit_num=1):
    blocks = []
        ...
        if self.fork_feat:
            ...
            for i_emb, i_layer in enumerate(self.out_indices):
                if i_emb == 0 and os.environ.get('FORK_LAST3', None):
                  ...
                  #  print(i_emb,embed_dims[i_emb],shape[i_emb],shape[i_emb])
                    layer = norm_layer([embed_dims[i_emb],shape[i_emb],shape[i_emb]])
...
def efficientformer_l1(pretrained=False, **kwargs):
    model = EfficientFormer(
        ...
        downsamples=[True, True, True, Truresolutione],
        vit_num=1,
        **kwargs)
